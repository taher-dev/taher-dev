<a title="Visit Taher's Portfolio" href="https://taher-portfolio.pages.dev">
   <img width="100%" src="./img/Github-Portfolio-Cover.png">
</a>

<div align="right">
    <p>
        <a title="Portfolio" target="_blank" href="https://taher-portfolio.pages.dev" target="_blank"><b>⭐ Portfolio</b></a> |
        <a title="LinkedIn" target="_blank" href="https://www.linkedin.com/in/taher-mahmud-monmoy" target="_blank"><b>💼 Linkedin</b></a> |
        <a title="Resume" target="_blank" href="https://taher-portfolio.pages.dev/assets/docs/taher-cv.pdf" target="_blank"><b>📜 Resume</b></a> 
    </p>
</div>

I am Taher Mahmud, currently pursuing my Bachelor’s degree in **Computer Science and Engineering** at Green University of Bangladesh.

![Skills](https://skillicons.dev/icons?i=js,react,vite)

![Skills](https://skillicons.dev/icons?i=css,tailwind,bootstrap,html,figma,mysql)

![Skills](https://skillicons.dev/icons?i=java,cpp,c,python,bash,matlab,latex,autocad,arduino&perline=9)

<p align="left">
	<a href="https://wakatime.com/@monmoy">
		<img 
			height="150px" 
			src="https://github-readme-stats-1chooo.vercel.app/api/wakatime?username=monmoy&layout=compact&langs_count=8&theme=nord"
         alt="Taher WakaTime Stats" />
	</a>
	<!-- <a href="https://leetcode.com/u/_uzumaki_">
		<img 
		height="150px" 
		src="https://leetcard.jacoblin.cool/_uzumaki_?theme=nord&font=PT%20Sans"
		alt="Taher LeetCode Stats" />
	</a> -->
</p>

### Technical Skills

<ul>
	<li>Web Development: Javascript(React), CSS(Tailwind, Bootstrap), HTML, PHP, MySQL</li>
    <li>UI/UX: Figma, Canva</li>
    <li>Version Control: Git, GitHub</li>
    <li>Programming: Java, C, C++, Python, Assembly, Arduino</li>
    <li>Additional: Office Applications, LATEX, MATLAB, Cisco Packet Tracer, AutoCAD,
 Tinkercad, GoPhish</li>
</ul>

<div align="center">
	<a 
		title="Total time coded since Jan 11, 2025"
		href="https://wakatime.com/@432046b1-b864-439e-b4d8-f27cc25d23e3" 
		target="_blank">
		<img 
			src="https://wakatime.com/badge/user/432046b1-b864-439e-b4d8-f27cc25d23e3.svg?style=for-the-badge" 
			alt="Total time coded since Jan 11, 2025" />
	</a>
</div>
